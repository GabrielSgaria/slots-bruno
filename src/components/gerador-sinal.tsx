...
export function GeradorSinal({ gameSlug }: GeradorSinalProps) {
    const game = gamesSinais.find(signal => signal.slug === gameSlug);
                ...
                <div className="flex flex-col">
                    <div className='h-full max-h-[150px] min-h-[100px] w-full'>
                        <Image
                            ...
                            quality={100}
                            priority={true}
                            src={imageMap[gameSlug]}
                            alt={gameSlug}
                            className="w-full h-[150px] object-cover"
                        />
                        <div
                            style={{ backgroundImage: `url(${imageMap[gameSlug]})` }}
                    ...
                    <div className='gameContent relative z-20 -top-1 flex flex-col min-h-[120px] justify-between backdrop-blur-sm '>
                        <div className='flex px-3 items-center justify-center gap-3 backdrop-blur-sm bg-zinc-950/30 h-[32px]'>
                            <span className=' text-sm leading-none uppercase py-1'>{gameSlug}</span>
                        </div>
                        <div className="px-3 py-6 flex flex-col text-center items-center justify-center gap-4 text-xs sm:text-[15px] h-full">
                            ...
                            <div className={cn("backdrop-blur-md bg-zinc-50/10 border border-zinc-950/10 px-3 py-1 w-full flex flex-row rounded-lg items-center shadow-inner shadow-black/90", textMap[gameSlug])}>
                                <div className="w-12 justify-center flex">
                                    <Image
                                        width={900}
                                        height={900}
